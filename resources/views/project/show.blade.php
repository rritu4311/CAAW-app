                ...
                <div class="p-6 bg-white dark:bg-gray-800">
                        ...
                        @if(($project->isOwnedBy(auth()->user()) || auth()->user()->isWorkspaceOwner($project) || auth()->user()->hasWorkspaceRole($project->workspace, ['user'])) && !$readOnly)
                            <div class="flex gap-2">
                                ...
                                <a href="{{ route('workflows.create', $project) }}"
                                    class="px-2 py-1 text-xs bg-green-400 text-black dark:text-white rounded hover:bg-green-500 dark:bg-green-900 inline-flex items-center gap-1"
                    ...
                    <div class="bg-white border rounded-lg p-6 mb-6">
                        <div class="flex items-center justify-between mb-4">
                            <h2 class="text-xl font-semibold text-gray-800">Approval Workflows</h2>
                            <!-- <a href="{{ route('workflows.create', $project) }}"
                                ...
                                @foreach($project->workflows as $workflow)
                                    <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div class="font-medium text-gray-900 dark:text-white">{{ $workflow->name }}</div>
                                        ...
                                        <div class="flex items-center gap-2">
                                            ...
                                            @else
                                                <span class="px-2 py-1 text-xs bg-gray-100 text-gray-700 dark:bg-gray-600 dark:text-gray-300 rounded-full">Inactive</span>
                                            @endif
                                            <a href="{{ route('workflows.edit', $workflow) }}" class="p-1 text-blue-600 hover:text-blue-700 transition-colors" title="Edit Workflow">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <form action="{{ route('workflows.destroy', $workflow) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete the workflow \"' + {{ json_encode($workflow->name) }} + '\"? This action cannot be undone.')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="p-1 text-red-600 hover:text-red-700 transition-colors">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                                    </svg>
                                                </button>
