<x-app-layout>
 ...
 <a href="{{ route('projects.show', $folder->project->id) }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-800">
 {{ $folder->project->name }}
 </a>
 <span class="text-gray-400 dark:text-gray-400">/</span>
 @foreach($breadcrumbs as $index => $breadcrumb)
 @if($index < count($breadcrumbs) - 1)
 <a href="{{ route('folders.show', $breadcrumb->id) }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-800">
 {{ $breadcrumb->name }}
 </a>
 <span class="text-gray-400 dark:text-gray-400">/</span>
 @else
 <span class="text-gray-700 dark:text-gray-300 font-medium">{{ $breadcrumb->name }}</span>
 @endif
 @endforeach
 </nav>
 ...
 @if($readOnly)
 <span class="mt-2 px-3 py-1 bg-yellow-100 dark:bg-yellow-900 text-yellow-800 rounded-full text-sm font-medium inline-block">Read-Only (Workspace Admin)</span>
 @endif
 </div>
 ...
 <!-- Success Message -->
 @if(session('success'))
 <div class="bg-green-100 dark:bg-green-900 border border-green-400 text-green-700 px-4 py-3 rounded mb-6">
 {{ session('success') }}
 </div>
 @endif
 ...
 class="px-4 py-2 bg-gray-300 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-400 :bg-gray-50 dark:bg-gray-7000 transition-colors">
 Cancel
 </a>
 </div>
 </form>
 ...
 <!-- File Upload Section -->
 @if(!$readOnly)
 <div id="dropZone" class="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-6 hover:border-blue-500 :border-blue-400 transition-colors mb-6 cursor-pointer">
 <div class="text-center">
 <svg class="mx-auto h-12 w-12 text-gray-400 dark:text-gray-400 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"></path>
 </svg>
 <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">Upload Files to "{{ $folder->name }}"</h3>
 ...
 <h2 class="text-xl font-semibold mb-4 text-gray-800 dark:text-gray-200">Folder Contents</h2>
 ...
 <x-folder-tree :folders="$folder->children" :projectId="$folder->project->id" :readOnly="$readOnly" :parentId="$folder->id" />
 </div>
 
 <!-- Files Section -->
 @if($folder->assets->count() > 0)
 <h3 class="text-lg font-semibold mb-3 text-gray-700 dark:text-gray-300">Files</h3>
 <div class="space-y-2">
 @foreach($folder->assets as $asset)
 <div class="flex items-center p-3 bg-white dark:bg-gray-800 border rounded-lg hover:shadow-md transition-shadow">
 ...
 <p class="text-xs text-gray-500 dark:text-gray-500">{{ $asset->formatted_size }} • {{ strtoupper($asset->file_type) }}</p>
 @if($asset->file_type === 'video')
 <div class="video-metadata text-xs text-gray-400 dark:text-gray-400 flex items-center gap-2 mt-1" data-video-src="{{ Storage::url($asset->file_path) }}">
 ...
 class="px-3 py-1.5 bg-gray-100 dark:bg-gray-900 hover:bg-gray-200 text-gray-700 dark:text-gray-300 text-xs font-medium rounded transition-colors">Download</a>
 ...
 <button type="submit" 
 class="px-3 py-1.5 bg-red-100 dark:bg-red-900 hover:bg-red-200 text-red-700 text-xs font-medium rounded transition-colors">
 Delete
 </button>
 </form>
 ...
 <div class="relative transform overflow-hidden rounded-lg bg-white dark:bg-gray-800 text-left shadow-xl transition-all sm:w-full sm:max-w-xl">
 <!-- Header -->
 <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 flex justify-between items-center border-b">
 <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100" id="crop-modal-title">Crop Image</h3>
 <button onclick="closeCropModal()" class="text-gray-400 dark:text-gray-400 hover:text-gray-500 dark:text-gray-500 focus:outline-none">
 ...
 <div class="relative max-h-[35vh] overflow-hidden bg-gray-100 dark:bg-gray-900 rounded-lg">
 ...
 <button onclick="setAspectRatio(1)" class="aspect-btn px-3 py-1.5 text-sm font-medium rounded-md bg-gray-200 text-gray-700 dark:text-gray-300 hover:bg-gray-300 transition-colors" data-ratio="1">
 1:1
 </button>
 <button onclick="setAspectRatio(16/9)" class="aspect-btn px-3 py-1.5 text-sm font-medium rounded-md bg-gray-200 text-gray-700 dark:text-gray-300 hover:bg-gray-300 transition-colors" data-ratio="16/9">
 16:9
 </button>
 <button onclick="setAspectRatio(4/3)" class="aspect-btn px-3 py-1.5 text-sm font-medium rounded-md bg-gray-200 text-gray-700 dark:text-gray-300 hover:bg-gray-300 transition-colors" data-ratio="4/3">
 4:3
 </button>
 <button onclick="setAspectRatio(3/2)" class="aspect-btn px-3 py-1.5 text-sm font-medium rounded-md bg-gray-200 text-gray-700 dark:text-gray-300 hover:bg-gray-300 transition-colors" data-ratio="3/2">
 3:2
 </button>
 </div>
 </div>
 ...
 <!-- Footer Actions -->
 <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 flex flex-wrap justify-between items-center gap-3 border-t">
 <div class="flex gap-2">
 <button onclick="cropper.rotate(-90)" class="p-2 text-gray-600 dark:text-gray-400 hover:bg-gray-200 rounded-lg transition-colors" title="Rotate Left">
 <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" />
 </svg>
 </button>
 <button onclick="cropper.rotate(90)" class="p-2 text-gray-600 dark:text-gray-400 hover:bg-gray-200 rounded-lg transition-colors" title="Rotate Right">
 <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 10h-10a8 8 0 00-8 8v2M21 10l-6 6m6-6l-6-6" />
 </svg>
 </button>
 <button onclick="cropper.reset()" class="p-2 text-gray-600 dark:text-gray-400 hover:bg-gray-200 rounded-lg transition-colors" title="Reset">
 <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
 </svg>
 </button>
 <button onclick="cropper.scaleX(-cropper.getData().scaleX || -1)" class="p-2 text-gray-600 dark:text-gray-400 hover:bg-gray-200 rounded-lg transition-colors" title="Flip Horizontal">
 <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" />
 </svg>
 </button>
 <button onclick="cropper.scaleY(-cropper.getData().scaleY || -1)" class="p-2 text-gray-600 dark:text-gray-400 hover:bg-gray-200 rounded-lg transition-colors" title="Flip Vertical">
 ...
 <div class="flex gap-2">
 <button onclick="closeCropModal()" class="px-4 py-2 bg-gray-200 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-300 transition-colors font-medium flex items-center gap-2">
 ...
 <div class="relative transform overflow-hidden rounded-xl bg-white dark:bg-gray-800 text-left shadow-2xl transition-all w-full max-w-xs">
 <!-- Header -->
 <div class="bg-gradient-to-r from-indigo-500 to-purple-600 px-4 py-3 flex justify-between items-center">
 <div class="flex items-center gap-2">
 <div class="bg-white dark:bg-gray-800/20 p-1.5 rounded-lg">
 ...
 <button onclick="closeMoveAssetModal()" class="text-white/80 hover:text-white focus:outline-none transition-colors rounded-lg p-1 hover:bg-white dark:bg-gray-800/10">
 ...
 <div class="mb-3">
 <div class="relative">
 <svg class="absolute left-2.5 top-1/2 transform -translate-y-1/2 w-3.5 h-3.5 text-gray-400 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
 </svg>
 <input type="text" id="folderSearchInput" placeholder="Search folders..." 
 class="w-full pl-8 pr-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all text-xs"
 oninput="filterFolders(this.value)">
 </div>
 </div>
 
 <!-- Folders List -->
 <div id="moveAssetFolders" class="border border-gray-200 dark:border-gray-700 rounded-lg max-h-60 overflow-y-auto bg-gray-50 dark:bg-gray-700 custom-scrollbar">
 <div class="px-4 py-8 text-center text-xs text-gray-400 dark:text-gray-400">
 ...
 <!-- Footer -->
 <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 flex justify-end gap-2 border-t border-gray-200 dark:border-gray-700">
 <button onclick="closeMoveAssetModal()" class="px-4 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 dark:bg-gray-700 transition-all font-medium text-xs focus:ring-2 focus:ring-gray-200 focus:outline-none">
 Cancel
 </button>
 </div>
 </div>
...
function setAspectRatio(ratio) {
 ...
 btn.classList.add('bg-gray-200', 'text-gray-700 dark:text-gray-300', 'hover:bg-gray-300');
 });
 
 const activeBtn = document.querySelector(`[data-ratio="${ratio === NaN ? 'free' : ratio}"]`) || 
 document.querySelector('[data-ratio="free"]');
 if (activeBtn) {
 activeBtn.classList.remove('bg-gray-200', 'text-gray-700 dark:text-gray-300', 'hover:bg-gray-300');
...
async function loadFoldersForModal(projectId, currentFolderId, container) {
 container.innerHTML = '<div class="px-4 py-8 text-center text-xs text-gray-400 dark:text-gray-400"><svg class="animate-spin h-5 w-5 mx-auto mb-2 text-indigo-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg><p class="font-medium">Loading folders...</p></div>';
...
function renderFoldersForModal(folders, currentFolderId, container, parentPath = '') {
 ...
 container.innerHTML = '<div class="px-4 py-8 text-xs text-gray-400 dark:text-gray-400 text-center"><svg class="w-8 h-8 mx-auto mb-2 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"/></svg><p class="font-medium">No folders available</p></div>';
 return;
 }
...
function renderFolderItemForModal(folder, currentFolderId, level, fullPath = '') {
 ...
 const disabledClass = isCurrent ? 'opacity-50 cursor-not-allowed bg-gray-100 dark:bg-gray-900' : 'hover:bg-indigo-50 hover:text-indigo-700 cursor-pointer group';
 ...
 class="w-full text-left px-3 py-2 text-xs text-gray-700 dark:text-gray-300 transition-all flex items-center border-b border-gray-200 dark:border-gray-700 last:border-b-0 ${disabledClass}"
 ...
 <span class="font-medium truncate">${folder.name} ${isCurrent ? '<span class="text-xs text-gray-400 dark:text-gray-400 ml-1">(current)</span>' : ''}</span>
 ${!isCurrent ? `<span class="text-xs text-gray-400 dark:text-gray-400 truncate group-hover:text-indigo-400">${fullPath}</span>` : ''}
 </div>
 ${!isCurrent ? `
