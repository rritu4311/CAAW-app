<x-app-layout>
 ...
 <p class="text-gray-500 dark:text-gray-500 text-lg">No projects found. Create your first project!</p>
 </div>
 @else
 @foreach($activeProjects as $project)
 <div onclick="window.location.href='{{ route('projects.show', $project) }}'"
 class="bg-white dark:bg-gray-800 rounded-lg shadow-md hover:shadow-xl transition-all duration-300 ease-in-out p-6 border border-gray-200 dark:border-gray-700 cursor-pointer">
 ...
 @endphp
 @if($pendingCount > 0)
 <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800 " title="{{ $pendingCount }} pending approval{{ $pendingCount > 1 ? 's' : '' }}">
 ...
 <button type="submit" 
 class="bg-blue-600 hover:bg-blue-700 :bg-blue-600 text-white p-2 rounded transition duration-200 border border-blue-700 "
 ...
 @method('DELETE')
 <button type="submit" 
 class="bg-red-600 hover:bg-red-700 :bg-red-600 text-white p-2 rounded transition duration-200 border border-red-700 "
 onclick="event.stopPropagation()">
 <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
