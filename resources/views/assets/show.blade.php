<x-app-layout>
 ...
 <a href="{{ route('projects.show', $asset->folder->project->id) }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-800">
 {{ $asset->folder->project->name }}
 </a>
 <span class="text-gray-400 dark:text-gray-400">/</span>
 
 @if($asset->folder->parent)
 <a href="{{ route('folders.show', $asset->folder->parent->id) }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-800">
 {{ $asset->folder->parent->name }}
 </a>
 <span class="text-gray-400 dark:text-gray-400">/</span>
 @endif
 
 <a href="{{ route('folders.show', $asset->folder->id) }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-800">
 {{ $asset->folder->name }}
 </a>
 <span class="text-gray-400 dark:text-gray-400">/</span>
 @endif
 <span class="text-gray-700 dark:text-gray-300 font-medium">{{ $asset->name }}</span>
 </nav>
 </div>

 @if(session('error'))
 <div class="mx-6 mt-4 p-4 bg-red-100 dark:bg-red-900 border border-red-400 text-red-700 rounded">
 {{ session('error') }}
 </div>
 @endif
 ...
 <div class="relative bg-gray-50 dark:bg-gray-700 rounded-lg p-4 flex items-center justify-center" style="min-height: calc(100vh - 80px);">
 <!-- Full-screen button -->
 ...
 <p class="text-gray-500 dark:text-gray-500 mb-6">This file type cannot be previewed directly in the browser.</p>
 ...
 <iframe src="{{ Storage::url($asset->file_path) }}"
 class="w-full rounded-lg border-0 bg-white dark:bg-gray-800"
 ...
 <p class="text-gray-500 dark:text-gray-500 mb-6">Preview not available for this file type.</p>
 <a href="{{ Storage::url($asset->file_path) }}" download="{{ $asset->name }}"
 class="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-colors">
 <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/>
 ...
 <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">Asset Information</h3>
 
 <!-- Status Badge -->
 <div class="mb-4">
 @if($asset->isDraft())
 <div class="flex items-center gap-2 px-3 py-2 bg-gray-200 rounded-lg">
 ...
 <span class="text-sm font-medium text-yellow-800 ">In Review</span>
 </div>
 ...
 <div class="flex items-center gap-2 px-3 py-2 bg-orange-100 /30 rounded-lg">
 <svg class="w-4 h-4 text-orange-600 " fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
 </svg>
 <span class="text-sm font-medium text-orange-800 ">Changes Requested</span>
 </div>
 @endif
 </div>
 ...
 @else
 <svg class="w-6 h-6 text-gray-500 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/>
 </svg>
 @endif
 </div>
 <div>
 <label class="text-gray-500 dark:text-gray-500 text-xs uppercase tracking-wider">File Type</label>
 <p class="text-gray-800 dark:text-gray-200 text-sm font-medium uppercase">{{ $asset->file_type }}</p>
 </div>
 </div>
 
 <div>
 <label class="text-gray-500 dark:text-gray-500 text-xs uppercase tracking-wider">File Name</label>
 <p class="text-gray-800 dark:text-gray-200 text-sm font-medium break-words">{{ $asset->name }}</p>
 </div>
 
 <div>
 <label class="text-gray-500 dark:text-gray-500 text-xs uppercase tracking-wider">Version</label>
 <p class="text-gray-800 dark:text-gray-200 text-sm font-medium">v{{ number_format($asset->version, 1) }}</p>
 </div>
 
 <div>
 <label class="text-gray-500 dark:text-gray-500 text-xs uppercase tracking-wider">File Size</label>
 <p class="text-gray-800 dark:text-gray-200 text-sm font-medium">{{ $asset->formatted_size }}</p>
 </div>
 
 <div>
 <label class="text-gray-500 dark:text-gray-500 text-xs uppercase tracking-wider">Uploaded By</label>
 <p class="text-gray-800 dark:text-gray-200 text-sm font-medium">{{ $asset->uploadedBy->name ?? 'Unknown' }}</p>
 </div>
 
 <div>
 <label class="text-gray-500 dark:text-gray-500 text-xs uppercase tracking-wider">Uploaded At</label>
 <p class="text-gray-800 dark:text-gray-200 text-sm font-medium">{{ $asset->created_at->format('M d, Y - g:i A') }}</p>
 </div>
 
 @if($asset->folder)
 <div>
 <label class="text-gray-500 dark:text-gray-500 text-xs uppercase tracking-wider">Folder</label>
 ...
 @if($asset->folder && $asset->folder->project)
 <div>
 <label class="text-gray-500 dark:text-gray-500 text-xs uppercase tracking-wider">Project</label>
 <p class="text-gray-800 dark:text-gray-200 text-sm font-medium">
 <a href="{{ route('projects.show', $asset->folder->project->id) }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-800">
 {{ $asset->folder->project->name }}
 </a>
 ...
 <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
 <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Annotations</h3>
 <div class="space-y-4">
 ...
 <span class="px-2 py-1 text-xs font-medium bg-blue-100 dark:bg-blue-900 text-blue-800 rounded-full">Acknowledged</span>
 @elseif($annotation->isResolved())
 <span class="px-2 py-1 text-xs font-medium bg-green-100 dark:bg-green-900 text-green-800 rounded-full">Resolved</span>
 @endif
 ...
 <div class="p-3 bg-gray-50 dark:bg-gray-700 rounded border border-gray-200 dark:border-gray-700 ">
 <div class="flex items-center gap-2 mb-1">
 <span class="text-xs font-medium text-gray-900 dark:text-gray-100 ">{{ $comment->user->name ?? 'Unknown' }}</span>
 <span class="text-xs text-gray-500 dark:text-gray-500 ">{{ $comment->created_at->format('M d, Y - g:i A') }}</span>
 </div>
 ...
 <span class="text-sm font-medium text-gray-900 dark:text-gray-100 ">{{ $comment->user->name ?? 'Unknown' }}</span>
 <span class="text-xs text-gray-500 dark:text-gray-500 ">{{ $comment->created_at->format('M d, Y - g:i A') }}</span>
 </div>
 <p class="text-sm text-gray-700 dark:text-gray-300 ">{{ $comment->text }}</p>
 </div>
 ...
 <tr class="border-b border-gray-100 hover:bg-gray-50 dark:hover:bg-gray-700 dark:bg-gray-700 :bg-gray-800">
 <td class="py-2 px-3">
 ...
 <select id="workflow-select" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 " onchange="document.getElementById('workflow_id_hidden').value = this.value">
 ...
 @else
 <div class="p-3 bg-yellow-50 /20 border border-yellow-200 rounded-lg">
 ...
 @endauth
 @elseif($asset->isApproved())
 <div class="p-3 bg-green-50 /20 border border-green-200 rounded-lg">
 <div class="flex items-center gap-2">
 ...
 @elseif($asset->isRejected())
 <div class="p-3 bg-red-50 /20 border border-red-200 rounded-lg">
 <div class="flex items-center gap-2">
 ...
 <p class="text-sm font-medium text-yellow-800 ">Changes Requested</p>
 </div>
 ...
 class="w-full inline-flex items-center justify-center gap-2 px-4 py-3 bg-gray-200 hover:bg-gray-300 :bg-gray-600 text-gray-700 dark:text-gray-300 rounded-lg font-medium transition-colors">
 <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div id="rejectModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black bg-opacity-50">
 ...
 <div class="p-6">
 <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Reject Asset</h3>
 ...
 class="px-4 py-2 bg-gray-200 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-300 :bg-gray-600 transition-colors">
 Cancel
 </button>
 <button type="submit"
...
<div id="requestChangesModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black bg-opacity-50 p-4">
 ...
 <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Request Changes</h3>
 ...
 <h4 class="text-sm font-medium text-gray-900 dark:text-gray-100 ">Add Annotation (Optional)</h4>
 <button type="button" onclick="toggleAnnotationFields()" class="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-800">
 + Add Annotation
 </button>
 </div>
 ...
 <p class="text-xs text-gray-500 dark:text-gray-500 mt-1">Click and drag on the image to select an area</p>
 </div>
 ...
 class="w-full px-2 py-1 text-sm border border-gray-300 dark:border-gray-600 rounded bg-gray-100 dark:bg-gray-900 "
...
<div id="annotationViewerModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black bg-opacity-50 p-4">
 ...
 <div class="flex items-center justify-between mb-4">
 <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 ">Annotation View</h3>
 <button type="button" onclick="document.getElementById('annotationViewerModal').style.display='none'"
 class="text-gray-500 dark:text-gray-500 hover:text-gray-700 dark:text-gray-300 :text-gray-200">
...
<div id="uploadVersionModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black bg-opacity-50 p-4">
 ...
 <div class="p-6">
 <div class="flex items-center justify-between mb-4">
 <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 ">Upload New Version</h3>
